...
export class Greetd extends Service {
    ...
    private async _send<R extends keyof Request>(req: R, payload: Request[R]): Promise<Response> {
        ...
        try {
            ...
            const dataBuffer = data.get_data();
            const length = new Uint32Array(dataBuffer as unknown as ArrayBuffer)[0];
            const res = await istream.read_bytes_async(length, GLib.PRIORITY_DEFAULT, null);
            const resData = res.get_data();
            return JSON.parse(this._decoder.decode(resData as unknown as ArrayBuffer)) as Response;
        } finally {
            connection.close(null);
        }
    }
}▉ yorha ▕▉  hyprland-yorha ▕ ᐉ sudo pacman -S xrandr

[sudo] password for drayuuta: 
error: target not found: xrandr

▉ yorha ▕▉  hyprland-yorha ▕ ᐉ swww init
